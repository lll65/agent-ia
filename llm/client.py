...
Fallback automatique : si Groq renvoie 429 (rate limit) et qu'une clé Gemini
est configurée, la requête bascule automatiquement sur Gemini. Si les deux
échouent, l'erreur est loggée clairement.
"""
import logging
...
def chat(messages: list, temperature: float = 0.7, num_ctx: int = 4096) -> str:
    ...
    Fallback automatique Groq → Gemini sur erreur 429.
    """
    ...
    if provider == "groq":
        try:
            ...
            if _is_rate_limit(e) and config.GEMINI_API_KEY:
                logger.warning(
                    f"[LLM] Groq 429 (rate limit) — bascule automatique sur Gemini "
                    f"({config.GEMINI_MODEL})."
                )
                try:
                    return _gemini_chat(messages, config.GEMINI_MODEL, temperature)
                except Exception as e2:
                    logger.error(
                        f"[LLM] ÉCHEC des deux providers. Groq: {e} | Gemini: {e2}"
                    )
                    raise RuntimeError(
                        f"Groq ET Gemini ont échoué. Groq(429)={e} ; Gemini={e2}"
                    ) from e2
